...
function MapNarratives({ styles, onSelectNarrative, svg, narrative, narratives, projectPoint, features }) {
  function getNarrativeStyle(narrativeId) {
    const styleName = narrativeId && narrativeId in styles ? narrativeId : 'default';
  ...
  function renderBetweenMarked(n) {
    ...
    if (narrativesExist) {
      for (let idx = 0; idx < n.steps.length; idx += 1) {
        ...
        if (lastMarked) {
          arrows.push(
            renderBetweenSteps(lastMarked, step, n.withLines ? { strokeWidth: '1px', stroke: step.colour } : {})
          );
        }
        lastMarked = step;
      }
