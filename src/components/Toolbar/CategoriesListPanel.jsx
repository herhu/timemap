...
export default ({ categories, activeCategories, onCategoryFilter, language }) => {
  function renderCategoryTree () {
    return (
      <div>
        {categories.map((cat) => {
          return (
            <li key={cat.category.replace(/ /g, '_')} className='filter-filter active' style={{ marginLeft: '20px' }}>
              <Checkbox
                label={cat.category}
